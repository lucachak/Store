...
from Subscription.models import Subscription, UserSubscription, SubscriptionStatus
...
def refresh_active_users_subscriptions(
    ...
    for obj in qs:
        ...
        if obj.stripe_id:
            sub_data = Core.billing.get_subscription(obj.stripe_id, raw=False)
...
def clear_dangling_subs():
    ...
    for customer_obj in qs:
        ...
        subs =  Core.billing.get_customer_active_subscriptions(customer_stripe_id)
        for sub in subs:
            existing_user_subs_qs = UserSubscription.objects.filter(stripe_id__iexact=f"{sub.id}".strip())
            if existing_user_subs_qs.exists():
                continue
            Core.billing.cancel_subscription(sub.id, reason="Dangling active subscription", cancel_at_period_end=False)
